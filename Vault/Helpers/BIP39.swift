...
extension BIP39 {
    ...
    private static func splitToWords(phrase: String) -> [String] {
        var words = [String]()
    ...
    static func validateSeedPhrase(phrase: String) -> BIP39InvalidReason? {
        ...
        let words = Self.splitToWords(phrase: normalizedPhrase)
        return Self.validateSeedPhrase(words: words)
    }

    static func validateSeedPhrase(words: [any StringProtocol]) -> BIP39InvalidReason? {
        if (words.count < 12) {
        ...
        for (index, word) in words.enumerated() {
            if let indexInList = Self.wordlist.firstIndex(of: String(word)) {
    ...
    static func phraseToBinaryEntropy(phrase: String) throws -> Data {
        let normalizedPhrase = phrase.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    static func binaryEntropyToWords(binaryEntropy: Data) throws -> [String] {
        // compute checksum
        ...
        let binaryPhrase = binaryEntropy.toBinaryString() + checksum
        ...
        for _ in 1...wordCount {
            ...
            words.append(Self.wordlist[Int(wordIndex)])
            startIndex = endIndex
        }
        return words
    }
